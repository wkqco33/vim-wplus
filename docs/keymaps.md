...
| 키 | 모듈 | 동작 |
|----|------|------|
| `<leader>p` | finder | 파일 찾기 |
| `<leader>b` | finder | 버퍼 찾기 |
| `<leader>m` | finder | 최근 파일 찾기 |
...
| `<C-a>` | Normal | multicursor | 모든 일치 항목 다중 선택 |
...
" 탐색
nnoremap <leader>p  <Plug>WplusFindFiles
nnoremap <leader>b  <Plug>WplusFindBuffers
